...
В репозитории доступна Postman-коллекция для тестирования всех API-эндпоинтов. Переменные коллекции:
 
- `base_url` — `http://localhost:8000/api`
- `token` — автоматически сохраняется после Register/Login
 
### Базовый сценарий тестирования
 
1. **Регистрация/логин** — получение токена.
2. **Создание справочников** — компании, сотрудники, курсы (с ценой).
3. **Создание учебной группы** — привязка к курсу, указание дат.
4. **Добавление участников** — привязка сотрудников к группе.
5. **Проверка расчётов** — стоимость группы = последняя цена курса × количество участников.
6. **Обновление прогресса** — изменение `completion_percent` у участников.
7. **Смена статуса группы** — проверка допустимых переходов state machine.
8. **Создание спецификации** — привязка групп, проверка расчёта НДС (22%).
9. **Диаграмма Ганта** — визуализация групп, проверка конфликтов, drag-and-drop.
10. **Загрузка сертификата** — upload PDF, скачивание.
11. **XML-импорт** — загрузка XML-файла, проверка создания записей и логов.
12. **Аналитика** — сводная и детальная статистика по компаниям.
